...
from terravision.models.dinov2_model import SegmentationHeadConvNeXt
from terravision.visualization.loss_graphs import plot_and_save_loss_graphs # Using shared logic if possible
...
def main():
    ...
    h = int(((540 / 4) // 14) * 14)
    lr = 1e-4
    n_epochs = 5  # We are adding 5 more epochs to our existing 10
    total_epochs = 15

    # Project paths
    script_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    output_dir = os.path.join(script_dir, 'outputs', 'train_stats')
    os.makedirs(output_dir, exist_ok=True)
    ...
    # Dataset paths
    dataset_base = os.path.join(script_dir, 'dataset', 'Offroad_Segmentation_Training_Dataset', 'Offroad_Segmentation_Training_Dataset')
    data_dir = os.path.join(dataset_base, 'train')
    ...
    repo_dir = os.path.join(script_dir, 'dinov2')
    backbone_model = torch.hub.load(repo_or_dir=repo_dir, source='local', model=backbone_name)
    backbone_model.eval()
    ...
    # Load previously trained model weights if they exist (to continue training)
    weights_path = os.path.join(script_dir, 'outputs', 'best_segmentation_head.pth')
    if os.path.exists(weights_path):
        print(f"Loading pretrained model weights from: {weights_path}")
        try:
        ...
        print("No pretrained weights found. Starting fresh training.")
    ...
    for i in epoch_pbar:
        epoch = i + 11  # Start display at 11 since we already did 10
        ...
        val_pbar = tqdm(val_loader, desc=f"Epoch {epoch}/{total_epochs} [Val]", 
                        leave=False, unit="batch")
        with torch.no_grad():
        ...
        # Save model after each epoch so evaluation can be run
        model_path = os.path.join(project_dir, 'outputs', 'segmentation_head.pth')
        os.makedirs(os.path.dirname(model_path), exist_ok=True)
        torch.save(classifier.state_dict(), model_path)
        
        if val_iou > best_iou:
            best_iou = val_iou
            best_model_path = os.path.join(project_dir, 'outputs', 'best_segmentation_head.pth')
            torch.save(classifier.state_dict(), best_model_path)
            print(f"New best model saved with IoU: {val_iou:.4f}")
        
        # Save a per-epoch checkpoint
